...
LIB_DIR = Path(__file__).parent / "data" / "library"
IDX_PATH = Path(__file__).parent / "data" / "tfidf_index.pkl"

SENT_SPLIT = re.compile(r"(?<=[\.!?])\s+")  # simple sentence splitter
...
def build_index() -> Index:
    ...
    texts = [c.text for c in chunks] or [""]  # avoid empty crash
    vectorizer = TfidfVectorizer(
        stop_words="english",
        max_features=50000,
        ngram_range=(1, 2),
...
def _best_snippet(page_text: str, query: str) -> str:
    # pick up to ~2-3 sentences that contain the best fuzzy match to the query
    ...
    best_idxs = [i for i, _ in scores[:3]]
    best_idxs.sort()
...
def search(query: str, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    sims = cosine_similarity(qv, idx.matrix)[0]  # shape: (N,)
    # gather top matches with score
    ...
    for pos, score in ranked:
        ...
        results.append({
            ...
            "score": float(score),
            "snippet": snip,
            "section_title": f"Page {ch.page}",  # next step: real sectioning
        })
    return results
