...
# ---------------- Runtime config (for Docker env) ----------------
@app.get("/config.js")
def config_js():
    # Prefer runtime envs passed via docker (-e ...)
    embed = os.getenv("ADOBE_EMBED_API_KEY") or os.getenv("VITE_ADOBE_EMBED_KEY", "")
    # Empty API_BASE => frontend should use same-origin (:8080)
    api_base = os.getenv("API_BASE", "")
    js = f"window.__CONFIG={{ADOBE_EMBED_API_KEY:{json.dumps(embed)},API_BASE:{json.dumps(api_base)}}};"
